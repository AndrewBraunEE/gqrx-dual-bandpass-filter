...
public:
    ...
    void setHiLowCutFrequencies1(int LowCut, int HiCut){ //Second Bandpass
        m1_DemodLowCutFreq = LowCut;
        m1_DemodHiCutFreq = HiCut;
        drawOverlay();
    }

    void getHiLowCutFrequencies1(int* LowCut, int* HiCut){
        *LowCut = m1_DemodLowCutFreq;
        *HiCut = m1_DemodHiCutFreq;
    }
    ...
    void    setSecondBandpassFreqDifference(int freq);
signals:
    void newCenterFreq(qint64 f);
    void newDemodFreq(qint64 freq, qint64 delta); /* delta is the offset from the center */
    void newLowCutFreq(int f);
    void newHighCutFreq(int f);
    void newFilterFreq(int low, int high);  /* substitute for NewLow / NewHigh */       //Surprisingly these signals were never even originally connected.
    void pandapterRangeChanged(float min, float max);
    void newZoomLevel(float level);

    void newDemod1Freq(qint64 freq, qint64 delta);
    void newFilter1Freq(int low, int high);  /* substitute for NewLow / NewHigh */
...
private:
    ...
    //Second Demod Bandpass Filter
    //TODO: Make the filters into QObjects that can be merged into plotter
    int         m1_Default_Offset;
    int         m1_CenterFreq;
    qint64      m1_DemodCenterFreq;
    int         m1_DemodHiCutFreq;
    int         m1_DemodLowCutFreq;
    int         m1_DemodFreqX;      //screen coordinate x position
    int         m1_DemodHiCutFreqX; //screen coordinate x position
    int         m1_DemodLowCutFreqX;//screen coordinate x position
